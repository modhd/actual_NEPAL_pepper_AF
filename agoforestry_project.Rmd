...
### Chance events
Water variable calculated with chance_event().
...
cashflow <- discount(final_income_AF, discount_rate = 10, calculate_NPV = FALSE)
...
NPV_tradeoff <- discount(tradeoff_benefit, discount_rate = 10, calculate_NPV = TRUE)
  
NPV_AF <- discount(final_income_AF, discount_rate = 10, calculate_NPV = TRUE)
  
  return(list(cashflow= cum_cashflow,trade_off= NPV_tradeoff,
              NPV_rubber= NPV_mono, NPV_AF= NPV_AF))
}

```

### NPV and EVPI calculation

``` {r}
# Montecarlo Simulation----
decisionSupport(inputFilePath = paste("data/AF_input_table.csv", sep = ","),
outputPath= paste("results/montecarlo_results", sep= ","),
write_table= TRUE,
welfareFunction = system_benefit,
numberOfModelRuns = 200,
functionSyntax = "plainNames")

#This is the montecarlo that is bringing error variable not found


#Old Montercarlo trial 
AF_mc_simulation <- mcSimulation(as.estimate(table),
                                         model_function = system_benefit,
                                         numberOfModelRuns = 200,
                                         functionSyntax = "plainNames")
write.csv(AF_mc_simulation, "./results/1_MC_simulation_results.csv")



# VoI ----
Mcall <- read.table(file= "./results/1_MC_simulation_results.csv",
                    header = TRUE, sep = ",")

mc <- Mcall[, c(27:57)] 

evpi <- multi_EVPI(mc, "y.trade_off", write_table = TRUE)

evpi

write.csv(evpi, "./results/evpi_results.csv")
...
plot_evpi(evpi, decision_vars= c("y.NPV_rubber", "y.NPV_AF"))
```


##Compound figures trial

```{r}
compound_figure(model = system_benefit, 
input_table = table, 
decision_var_name = "NPV_tradeoff",
cashflow_var_name = "cum_cash_flow",
model_runs = 200,
