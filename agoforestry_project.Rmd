...
cum_cashflow <- cumsum(cashflow)
  ...
  ### NPV 
NPV_mono <- discount(final_income_mono, discount_rate = discount_rate, calculate_NPV = TRUE)

NPV_tradeoff <- discount(tradeoff_benefit, discount_rate = discount_rate, calculate_NPV = TRUE)
  
NPV_AF <- discount(final_income_AF, discount_rate = discount_rate, calculate_NPV = TRUE)
...
#TO BE CORRECTED DUE TO LABEL PROBLEM

```{r}
#To get names of y and x 
...
## Expected value of perfect information EVPI with recommendation
...
#EVPI without information
```{r}
mcresults_table <- data.frame(AF_mc_simulation$x,
                          AF_mc_simulation$y[26:28])

evpi<- multi_EVPI(mc= mcresults_table,
                  first_out_var = "trade_off", write_table = FALSE, outfolder = ./results/evpi_results)


plot_evpi<-plot_evpi(evpi,
          decision_vars = "NPV_AF",
          new_names= "Rubber and Black pepper",
          unit= "NPR",
          bar_color= "yellow4",
          base_size=15)
...
#TO BE CORRECTED DUE TO LABEL NAME

```{r}
compound_figure(model = system_benefit, 
input_table = table, 
decision_var_name = "NPV_AF",
cashflow_var_name = "cashflow",
model_runs = 200,
